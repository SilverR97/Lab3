...
class MoveAppViewModel: ObservableObject {
    ...
    func saveData() {
        ...
        let combinedData: [String: [[String: Double]]] = [
            ...
            "Emwa Angle": formatAngleHistory(angleHistory),
            "Complementary angle": formatAngleHistory(compAngleHistory)
    ...
    private func processAngles(from data: SensorData) {
        // Get both EMWA and complementary filtered angles
        let angleDataPair = self.mainModel.processSensorData(data, accelData: accelerometerData, gyroData: gyroscopeData)
        
        // Use EMWA-filtered angles
        self.filteredAngles = angleDataPair.emwaFilteredAngles
        self.updateAngleHistory(with: angleDataPair.emwaFilteredAngles)
        
        // Use complementary-filtered angles
        self.compFilteredAngles = angleDataPair.compFilteredAngles  // Update the complementary-filtered angles
